...
const PublicRouteProtector: React.FC<Props> = ({ children }) => {
  const { isLoggedIn, setIsLoggedIn, setOrganizationName, organizationName } = useAuthStore() // Zustand state for auth
  ...
  if (isLoggedIn) {
    return <Navigate to={`/${organizationName}/projects`} state={{ from: location }} replace />
  }
